...
class Register extends Component {
	...
	submitRegister(e) {
		...
		if (email && password) {
			this.props.register({firstName, lastName, userName, email, password, confirmPassword});
		}
	}
	...
	render(){
		...
		return(
			...
			<div className={'form-group' + (submitted && !password ? ' has-error' : '')}>
					...
					<input type="password" id="ConfirmPassword" className="form-control input-shadow" placeholder="Enter Password" value={this.state.confirmPassword}
					onChange={this.handleInputChange} name="ConfirmPassword"/>
					{ submitted && !firstName && <div className="help-block">confirmPassword is required</div> }
					</div>
